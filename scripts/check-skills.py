...
def cli_surface():
    ...
    binary = os.environ.get("VYQL_BIN") or shutil.which("vyql")
    if not binary or not os.path.exists(binary):
        return None, None
    # The binary resolves its own data directory; this repository has none.
    env = dict(os.environ)
...
def check(skill_dir):
    ...
    if subs is None:
        print("  (skipped command check: no vyql on PATH; set VYQL_BIN)")
        return
    body = text
