...
already. On top of those I would skip:
  ...
  **/*_templ.go    generated templates, one huge function each
  sdks/, **/*.pb.go generated clients and stubs, cannot be hand-edited
...
**Generated code and test files are the two largest noise sources on a real
codebase, and both are worth proposing to exclude.** Generated clients and stubs
carry findings at several times the density of hand-written code, and most repos
forbid editing them, so a finding there cannot be fixed in place — detect them by
marker (`// Code generated by ... DO NOT EDIT.`, an `@generated` header,
`openapi-generator` banners) and offer to skip or down-rank them. Test files
inflate the count through cross-test taint hubs that join unrelated call graphs.
Propose excluding both, say why, and let the user keep them in if they want.
...
**Surface the review flags, not only the findings.** `-flags with` adds attention
and review flags that the findings list does not carry. On a noisy scan these are
often the highest-signal output: a `code.ObjectLookupByUserInput` review flag is a
precise IDOR question about multi-tenant object access — the class a taint engine
cannot prove and a human must judge — and it is worth more than a page of
name-matched criticals. List the attention-kind flags alongside the findings and
say which questions they raise, rather than letting them sit under the noise.
...
VyQL's errors arrive as families, not one at a time. One mislabelled binding
produces the same wrong finding at every site it matches, so a scan with hundreds
of findings usually has only a handful of distinct root causes behind them. Triage
the root causes, not the findings. `references/blindspots.md` is the taxonomy of
what VyQL gets wrong and how to settle each family cheaply; read it before
verifying a scan that returned many findings.

Work in this order, and read `references/verify.md` for how:

1. **Group by CWE, then collapse by root cause.** CWE is the axis a reader acts
   on. Within a CWE, findings that share a rule and sink API, or a single taint
   source, or an identical unsatisfiable `unless` line, are one root cause and get
   one verdict. Name the count each verdict retires.
2. **Apply the structural signals first**, from `references/blindspots.md`. Each
   settles a whole cluster from the scan output plus at most one grep, before any
   source is read: a CWE the language cannot express, a sink that is a name
   collision, a single handle tainting everything, an orphaned sink whose `unless`
   can never be satisfied, a rule on the wrong surface, an existing suppression.
   Most large clusters end here. This is what keeps triage fast at scale.
3. **Send survivors to source reading.** Only clusters the structural signals did
   not settle need the taint questions. Fan out one subagent per surviving
   cluster, four at most; clusters past the cap are deferred by name, never
   dropped.
4. **Where does this code live?** Surface and source trust. A path traversal in
   `testdata/` is not a vulnerability, a plugin loader that evals is doing its job,
   and a server-posture rule on a client SDK is on the wrong side of the boundary.
5. **What fired, and why.** `vyql explain .` gives the proof tree and the `unless`
   lines, which are the fix list.
6. **Three questions**, all of them: is the source really attacker-controlled,
...
- A false positive is a finding about VyQL, not a nuisance. Report the binding
  gap.
- Findings arrive in families from a few mislabelled bindings. Retire the family
  with one verdict and name the count; do not present one root cause as hundreds
  of independent problems.
- Reducing noise is not the same as calling code safe. An orphaned sink, a cleared
  dependency, and an unmodelled framework are unadjudicated, not clean. Say so.
- A silent false negative is worse than a loud false positive, because nobody
  reports it. Over an attacker-facing surface, "no findings" means "not
  adjudicated" until a person has read the important flows.
